...
export default function ReportsPage() {
  ...
  const [sparklineView, setSparklineView] = useState<"byExpenses" | "byIncome">(
    "byExpenses",
  );
  ...
  const selectedRange =
    rangeOptions.find((r) => r.key === rangeKey) ?? rangeOptions[0];
  ...
  const { data, isLoading } = useQuery<ReportResponse>({
    queryKey: [
      ...
      period ?? null,
      interval,
      isCashFlow ? cashFlowBaseline : false,
      activeAccountIds,
      activeWalletIds,
    ],
    queryFn: () =>
  ...
  const chartData = trend.map((dp) => {
    ...
    const breakdowns =
      meta?.type === "net_worth"
        ? Object.fromEntries(
            Object.entries(dp.breakdowns).map(([k, v]) => [
              k,
              NEGATIVE_SERIES.has(k) ? -v : v,
            ]),
          )
        : dp.breakdowns;
    return {
  ...
  const changeColor =
    (summary?.change_amount ?? 0) >= 0 ? "text-emerald-600" : "text-rose-500";
  ...
  const activeCompositionGroups: Set<string> | null = (() => {
    if (compositionView === "assetsAndAccounts")
      return new Set(["accounts", "assets"]);
  ...
  const innerDonutData = (() => {
    ...
    return compositionBreakdownData
      .filter(
        (b) =>
          !excludedKeys.has(b.key) &&
          (!activeCompositionGroups ||
            activeCompositionGroups.has(groupOf(b.key))),
      )
      .map((b) => ({
  ...
  const outerDonutData = (() => {
    ...
    const otherLabel = (g: string) =>
      g === "accounts"
        ? t("reports.otherAccounts")
        : g === "assets"
          ? t("reports.otherAssets")
          : g === "liabilities"
            ? t("reports.otherLiabilities")
            : g === "income"
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-1 mb-5 border-b border-border">
        {REPORT_TABS.map((tab) => (
          <button
            key={tab.key}
            onClick={() => {
              if (tab.enabled) handleSelectTab(tab.key);
            }}
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-0.5">
                  {mask(
                    `${changePrefix}${formatCurrency(summary?.change_amount ?? 0, userCurrency, locale)}`,
                  )}{" "}
                  {t(
                    meta?.type === "cash_flow"
                      ? "reports.vsToday"
            ...
            <div className="flex items-center gap-3">
              ...
              {meta.type === "cash_flow" && (
                <div className="flex items-center gap-1.5">
                  ...
                  />
                  <span className="text-[11px] text-muted-foreground">
                    {meta.baseline_active
                      ? t("reports.forecastBaseline")
                      : t("reports.forecast")}
                  </span>
                </div>
              )}
            ...
            meta?.type === "cash_flow" ? (
              (() => {
                const startingBalance =
                  summary?.breakdowns.find((b) => b.key === "startingBalance")
                    ?.value ?? 0;
                return (
                            ...
                            <div style={tooltipStyle} className="px-3 py-2">
                              ...
                              {inflow > 0 && (
                                ...
                                >
                                  ...
                                  {privacyMode
                                    ? MASK
                                    : `+${formatCurrency(inflow, userCurrency, locale)}`}
                                </p>
                              )}
                              {outflow > 0 && (
                                <p
                                ...
                                >
                                  ...
                                  {privacyMode
                                    ? MASK
                                    : `-${formatCurrency(outflow, userCurrency, locale)}`}
                                </p>
                              )}
                            </div>
                          );
                ...
                >
                  ...
                  <Tooltip
                    formatter={(value?: number, name?: string) => [
                      privacyMode
                        ? MASK
                        : formatCurrency(value ?? 0, userCurrency, locale),
                      name === "value"
                        ? t("reports.netIncome")
                        : t(`reports.${name ?? ""}`, {
                            defaultValue: name ?? "",
                      ...
                      return (
                        <div style={tooltipStyle} className="px-3 py-2">
                          ...
                          <p className="text-xs" style={{ color: "#6366F1" }}>
                            ...
                            {privacyMode
                              ? MASK
                              : formatCurrency(value, userCurrency, locale)}
                          </p>
                          {change !== null && (
                            <p
                              className="text-xs"
                            ...
                            >
                              ...
                              {privacyMode
                                ? MASK
                                : `${changeSign}${formatCurrency(change, userCurrency, locale)}`}
                            </p>
                          )}
                        </div>
                      );
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 items-start">
        ...
        <div
          className={`rounded-xl border shadow-sm transition-colors ${selectedDate ? "bg-primary/5 border-primary/50" : "bg-card border-border"}`}
        >
            ...
            {isLoading ? (
              <div className="flex flex-col items-center px-4 py-2">
                ...
                <div className="flex flex-wrap justify-center gap-x-3 gap-y-1.5 mt-3">
                  {Array.from({
                    length: activeTab === "net_worth" ? 3 : 2,
                  }).map((_, i) => (
              ...
              (() => {
                ...
                const donutTotal = innerDonutData.reduce(
                  (s, d) => s + d.value,
                  0,
                );
                return (
                  <div className="flex flex-col items-center">
                            ...
                            return (
                              ...
                              >
                                ...
                                <p className="text-xs">
                                  {privacyMode
                                    ? MASK
                                    : `${formatCurrency(v, userCurrency, locale)} (${pct}%)`}
                                </p>
                                      ...
                                      return (
                                        ...
                                        >
                                          ...
                                          <span className="text-xs text-muted-foreground shrink-0">
                                            {privacyMode
                                              ? MASK
                                              : `${formatCurrency(child.value, userCurrency, locale)} (${childPct}%)`}
                                          </span>
                                        </div>
                                      );
                                    })}
                      ...
                      >
                        ...
                        <span className="text-base font-bold text-foreground tabular-nums">
                          {mask(
                            formatCompact(
                              snapshotTrendPoint
                                ? compositionView === "netWorth" ||
                                  compositionView === "net" ||
                                  !compositionView
                                  ? snapshotTrendPoint.value
                                  : innerDonutData.reduce(
                                      (s, d) => s + d.value,
                                      0,
                                    )
                                : compositionView === "netWorth" ||
                    ...
                    <div
                      key={compositionView}
                      className="flex flex-col items-center gap-1 px-3 mt-1 w-full"
                    >
                      ...
                      {hasOuter &&
                        (() => {
                          ...
                          const hiddenCount =
                            compositionDetail.length - visible.length;
                          return (
        ...
        <div
          className={`lg:col-span-2 rounded-xl border shadow-sm transition-colors ${selectedDate && meta?.type !== "income_expenses" ? "bg-primary/5 border-primary/50" : "bg-card border-border"}`}
        >
          <div className="px-5 pt-5 pb-2 flex items-center justify-between">
            <p className="text-sm font-semibold text-foreground">
              {meta?.type === "income_expenses"
            ...
            {meta?.type === "income_expenses" &&
              (() => {
                const groupKey =
                  sparklineView === "byIncome" ? "income" : "expenses";
                const allItems = (data?.category_trend ?? []).filter(
                  (c) => c.group === groupKey,
                );
                  ...
                  <div className="flex items-center gap-2">
                    {hasPagination && (
                      <div className="flex items-center gap-0.5">
                        <button
                          onClick={() =>
                            setSparklinePage((p) => Math.max(0, p - 1))
                          }
            ...
            <div className="pb-4 overflow-hidden">
              {isLoading ? (
                ...
                (() => {
                  const groupKey =
                    sparklineView === "byIncome" ? "income" : "expenses";
                  const allGroupItems: CategoryTrendItem[] = (
                    data?.category_trend ?? []
                  ...
                  return (
                    ...
                    >
                      {pages.map((pageItems, pageIdx) => (
                        ...
                        >
                          {pageItems.map((item) => {
                            const sparkData = item.series.map((s) => ({
                              date: s.date,
                              v: s.value,
                            }));
                ...
                <ResponsiveContainer width="100%" height="100%">
                  ...
                  >
                    ...
                    {(() => {
                      ...
                      const positiveKeys = isNetWorth
                        ? filteredSeries.filter(
                            ({ key }) => !NEGATIVE_SERIES.has(key),
                          )
                        : filteredSeries;
                      const negativeKeys = isNetWorth
                        ? filteredSeries.filter(({ key }) =>
                            NEGATIVE_SERIES.has(key),
                          )
                        : [];
